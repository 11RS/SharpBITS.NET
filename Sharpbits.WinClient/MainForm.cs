...
namespace SharpBits.WinClient
{
    public partial class MainForm : Form
    {
        ...
        private ToolStripMenuItem ctxmiBrowserIntegration;
        private ToolStripSeparator toolStripMenuItem2;
        private ToolStripMenuItem toolStripMenuItem3;
        private ToolStripMenuItem tsmiBrowserIntegration;
        ...
        public MainForm()
        {
            ...
            this.ctxmiBrowserIntegration.CheckState = this.tsmiBrowserIntegration.CheckState = Settings.Default.EnableIEExtension ? CheckState.Checked : CheckState.Unchecked;
            ...
            try
            {
                ...
                if (key != null)
                {
                    ...
                    if (Settings.Default.EnableIEExtension)
                    {
                        ...
                        this.ctxmiBrowserIntegration.Text = this.tsmiBrowserIntegration.Text = "Browser Integration enabled";
                    }
                    else
                    {
                        key.DeleteSubKey("&Download using SharpBITS", false);
                        File.Delete(Path.Combine(directoryName, str3));
                        this.ctxmiBrowserIntegration.Text = this.tsmiBrowserIntegration.Text = "Browser Integration disabled";
                    }
                }
                else
                {
        ...
        {
            this.jobListControl.ShowAllJobs();
        }
        ...
        private void ctxmiMainExit_Click(object sender, EventArgs e)
        {
            Settings.Default.Save();
            Application.Exit();
        }

        private void tsmiBrowserIntegration_Click(object sender, EventArgs e)
        {
            Settings.Default.EnableIEExtension = this.tsmiBrowserIntegration.Checked;
            this.CheckIEMenuExtension();
            Settings.Default.Save();
        }
        ...
        {
            base.Hide();
            this.ctxSystrayShow.Enabled = true;
            if (Settings.Default.MinimizeWarning)
            {
                this.systray.BalloonTipClicked += new EventHandler(this.systray_BalloonTipClickedMinimizeWarning);
        ...
        private void ctxSystrayShow_Click(object sender, EventArgs e)
        {
            this.ShowForm();
        }
    }
